...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between items-center p-4 mx-6 relative w-full">
      <div className="flex items-center">
        <Image src="/RF.svg" alt="شعار" width={50} height={50} />
      </div>

     
      <div className={`hidden md:flex flex-1 justify-center items-center ${isMobileMenuOpen ? "hidden" : ""}`}>
        <div className="flex gap-10 items-center">
          ...
          <div className="relative">
            <button
              ...
              className="font-itim text-lg md:text-xl"
            >
              Recipes
            </button>
            {isRecipesMenuOpen && (
      ...
      <div className="hidden md:flex">
        <Link href={"/Favorite"}>
          <button className="font-itim text-lg md:text-xl bg-slate-500 rounded-xl p-2 text-white">
            Favorite food
          </button>
        </Link>
      </div>
      ...
      <div className={`fixed inset-0 bg-white transition-transform transform ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"} md:hidden z-50`}>
        <div className="flex flex-col items-center p-4 relative w-full min-w-[300px] max-w-full">
          ...
          <div className="relative">
            <button
              onClick={() => { toggleRecipesMenu(); handleLinkClick(); }}
              className="font-itim text-lg md:text-xl py-2 hover:text-red-600"
            >
              Recipes
            </button>
            {isRecipesMenuOpen && (
